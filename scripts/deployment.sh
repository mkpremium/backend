...
source ~/.nvm/nvm.sh
mkdir -p ${deploy_dir}
tar xzf ${dist_file} -C ${deploy_dir} --strip-components=2 > /dev/null
rm ${dist_file}
cd ${deploy_dir}
nvm install
npm install
