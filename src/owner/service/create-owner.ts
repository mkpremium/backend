...
export async function createOwner (entityManager: EntityManager, cmd: CreateOwnerCommand): Promise<[ Owner, Person ]> {
  if (cmd.person.name !== `${cmd.person.firstName} ${cmd.person.firstSurname}`) {
    logger.warning(`Owner name ${cmd.person.name} does not match first name ${cmd.person.firstName} and first surname ${cmd.person.firstSurname}`, cmd)
  }
  if (!cmd.buildingId) {
    logger.warning('Owner does not have a building', cmd)
  }
