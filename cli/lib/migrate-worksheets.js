...
import parse from '../../src/migration/models/cross-table';

export async function migrateWorksheets(inputFile, bucket) {
  const worksheets = new MigrateWorksheets(inputFile, bucket);
  return worksheets.run();
}

class MigrateWorksheets extends MigrateModelV3 {
  constructor(inputFile, bucket, opts) {
    super(inputFile, bucket, opts);
    this.ownersCount = 0;
    this.buildings = 0;
    this.owners = [];
  ...
  async parseToData(data, row) {
    const relation = parse(data);
    if (this.isSameBuilding(relation)) {
      this.addOwner(relation);
    } else {
      return this.createWorksheet(relation);
    }
  }
